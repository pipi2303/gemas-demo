      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {!loading && rows.map((e, idx) => (
              <tr key={e.journal_line_id || e.id || `${e.journal_id || 'entry'}-${e.line_number ?? idx}-${idx}`} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/60">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {!loading && rows.map((r, idx) => {
              ...
              return (
                <tr key={r.account_id || r.id || `${r.account_code || 'acc'}-${idx}`} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/60">
      ...
      <FinancePageHeader
        ...
        secondaryActions={
          <div className="flex gap-1 bg-slate-100/90 p-1 rounded-xl border border-slate-200/80">
            ...
            >
              <ScrollText className="w-3.5 h-3.5" /> Mutasi Akun
            </button>
            <button
        ...
        infoStrip={[
          {
            label: 'Total Akun',
            value: `${accounts.length} Akun COA`,
            color: 'emerald',
          },
          {
            label: 'Status Neraca Saldo',
            value: 'Seimbang (Σ D = Σ K)',
            color: 'teal',
          },
          {
            label: 'Kepatuhan GL',
            value: 'Prinsip Posting Permanen',
            color: 'indigo',
          },
          {
            label: 'Audit Trail',
            value: 'Terkoneksi ke Voucher',
            color: 'sky',
          },
        ]}
      />
